...
class CourseTreatmentMedicationBarChart extends StatelessWidget {
  final List<CourseTreatment> courses;

  const CourseTreatmentMedicationBarChart({super.key, required this.courses});
  ...
  Widget build(BuildContext context) {
    ...
    final List<CourseTreatmentEntry> allEntries = [];
    for (final course in courses) {
      allEntries.addAll(course.entries);
    }
    ...
    for (final entry in allEntries) {
      final typeName = entry.medicationType.displayName(context);
      typeCounts.update(typeName, (existing) => existing + 1, ifAbsent: () => 1);
    }
